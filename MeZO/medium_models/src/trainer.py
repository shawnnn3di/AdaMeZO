...
class Trainer(LinearHeadTrainer):
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        assert not self.args.zo_by_layer, "Zero-order by layer is not supported yet"
        assert self.args.zero_order_optim, "Must be zero order optimization"
        args = self.args
        ...
        if self.args.norm_cache != 'None':
            self.norm_cache = pk.load(open(self.args.norm_cache, 'rb'))
        ...
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            # if (self.tolerance > 20) and (len(self.eval_loss_history) > 1):
            ...
            if args.dump_norm_cache != 'None':
                pk.dump(self.gnorm_history, open(args.dump_norm_cache, 'wb'))
                ...
                if self.args.zero_order_optim:
                    ...
                    if self.args.zo_by_layer:
                        ...
                        for _ in range(num_zs):
                            ...
                            self.seed_history.append(self.zo_random_seed)
                            self.projection_history.append(self.projected_grad)
                    ...
                    # if not using the trainer, the updates are resampled and directly applied to the parameters
                        ...
                        if args.hw < 2 or self.total_steps <= args.hw:
                            # Reset the random seed for sampling zs
                            
                            if args.norm_cache != 'None':
                                ...
                                for name, param in self.named_parameters_to_optim:
                                    ...
                                    # g = g / torch.norm(g) * self.norm_cache[self.total_steps] / self._get_learning_rate()
                                    ...
                                    total_g_norm += (torch.norm(g.to(torch.float32)) ** 2).item()
                                    ognorm = (total_g_norm ** 0.5) * self._get_learning_rate()
                                ...
                                for name, param in self.named_parameters_to_optim:
                                    ...
                                    if ognorm < 1e-5:
                                        ognorm = 1e-5
                                    g *= self.norm_cache[self.total_steps] / ognorm
                                    if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                                        param.data = param.data - self._get_learning_rate() * (g + args.weight_decay * param.data)
                                    else:
                                        param.data = param.data - self._get_learning_rate() * (g)
                                    total_g_norm += (torch.norm(g.to(torch.float32) * self._get_learning_rate()) ** 2).item()
                                    
                                tgnorm = (total_g_norm ** 0.5)
                                logger.info('mynorm, %d, %.4e, m, %.4e, v, %.4e' % (self.total_steps, 
                                                                                    tgnorm,
                                                                                    -1, 
                                                                                    -1, 
                                                                                    ))
                            ...
                            state_list = [None] * args.hw
                            total_g_norm = 0.0
                            beta1 = args.beta1
                            beta2 = args.beta2
                            for name, param in self.named_parameters_to_optim:
                                temp_grad = torch.zeros(size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                                temp_hess = torch.zeros(size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                                for i in range(args.hw):
                                    idx = -(args.hw - i)
                                    if state_list[i] is None:
                                        self.g.manual_seed(self.seed_history[idx])
                                    else:
                                        self.g.set_state(state_list[i])
                                    z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype, generator=self.g)
                                    state_list[i] = self.g.get_state()
                                    temp_grad += self.projection_history[idx] * (beta1 ** (args.hw - 1 - i)) * z
                                    temp_hess += (self.projection_history[idx] ** 2) * (beta2 ** (args.hw - 1 - i)) * (z ** 2)
                                if beta2 == 0:
                                    temp_hess = torch.ones_like(temp_hess)
                                g = temp_grad / (temp_hess ** 0.5 + 1e-5) * 100
                                total_g_norm += (torch.norm(g.to(torch.float32) * self._get_learning_rate()) ** 2).item()
                                if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                                    param.data = param.data - self._get_learning_rate() * (g + args.weight_decay * param.data)
                                else:
                                    param.data = param.data - self._get_learning_rate() * g
                            tgnorm = (total_g_norm ** 0.5)
                            self.gnorm_history.append(tgnorm)
                            logger.info('mynorm, %d, %.4e, m, %.4e, v, %.4e' % (self.total_steps, 
                                                                                    tgnorm,
                                                                                    -1, 
                                                                                    -1, 
